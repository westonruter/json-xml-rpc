...
class RPCServer {
	...
	var $isUsingIncludedFunctions = false;
	...
	**END PHP5*/
	...
	function encodeJson($value){
		...
		//case 'array':
			...
			if(RPCServer::isVector($value)){
				...
				return $json . ']';
				#return '[' . join(",", array_map('RPCServer::encodeJson', $value)) . ']';
			}
			else {
				$json = '{';
				$count = 0;
		...
		switch($tokens[$i]){
								...
								switch($obj[$name]['__jsonclass__'][0]){
									...
									case 'Date':
										if(is_array($obj[$name]['__jsonclass__'][1])){
											...
											print $param . "##\n";
											if(is_numeric($param))
												$obj[$name] = RPCServer::timestampToDateTime((double) $param);
											else
								...
								switch($obj['__jsonclass__'][0]){
									case 'Date':
										...
										if(get_class($param) == 'DateTime')
											...
											$jsonObj = RPCServer::timestampToDateTime((double) $param);
										else
			...
			default:
				...
				if($c == '"'){
					...
					{
						return RPCServer::timestampToDateTime((double) $matches[1]);
					}
				...
				#Number
					...
					return RPCServer::stringToNumber($tokens[$i]);
				}
	...
	function encodeXmlRpc($value){
		...
		//case 'array':
			...
			if(RPCServer::isVector($value)){
				$xml .= '<array><data>';
				#$xml .= join("", array_map('RPCServer::encodeXmlRpc', $value));
				for($i = 0; $i < count($value); $i++){
	...
	function decodeXmlRpc($valueEl){ #function parseValue($valueEl){
		if(count($valueEl->child_nodes) == 1 &&
		   $valueEl->child_nodes[0]->node_type == 3)
		{
			return $valueEl->child_nodes[0]->node_value;
		}
		for($i = 0; $i < count($valueEl->child_nodes); $i++){
			if($valueEl->child_nodes[$i]->node_type == 1){
				$typeEl = $valueEl->child_nodes[$i];
				switch($typeEl->node_name){
					case 'i4':
					case 'int':
						...
						if(!preg_match("/^[-\+]?\d+$/", $typeEl->first_child->node_value))
							trigger_error("XML-RPC Parse Error: The value provided as an integer '" . $typeEl->first_child->node_value . "' is invalid.");
						
						$double = (double) $typeEl->first_child->node_value;
						$int = (int) $typeEl->first_child->node_value;
					...
					case 'double':
						...
						if(!preg_match("/^[-\+]?\d+(\.\d+)?$/", $typeEl->first_child->node_value))
							trigger_error("XML-RPC Parse Error: The value provided as a double '" . $typeEl->first_child->node_value . "' is invalid.");
						return (double) $typeEl->first_child->node_value;
					case 'boolean':
						if($typeEl->first_child->node_value != '0' && $typeEl->first_child->node_value != '1')
							trigger_error("XML-RPC Parse Error: The value provided as a boolean '" . $typeEl->first_child->node_value . "' is invalid.");
						return (bool) $typeEl->first_child->node_value;
					case 'string':
						if(!$typeEl->first_child)
							return "";
						return (string) /*utf8_decode*/($typeEl->first_child->node_value);
					case 'dateTime.iso8601':
						#try {
							$date = new DateTime($typeEl->first_child->node_value);
						#}
						#catch(Exception $e){
						#	trigger_error("XML-RPC Parse Error: The value provided as a dateTime.iso8601 '" . $typeEl->first_child->node_value . "' is invalid.");
						#}	
						return $date;
					case 'base64':
						return base64_decode($typeEl->first_child->node_value);
					case 'nil':
						return null;
					case 'struct':
						#A <struct> contains <member>s and each <member> contains a <name> and a <value>.
						$struct = array();
						#$memberEl = $typeEl->first_child;
						for($j = 0; $memberEl = $typeEl->child_nodes[$j]; $j++){
							if($memberEl->node_type == 1 && $memberEl->node_name == 'member'){
								$name = '';
								$valueEl = null;
								for($k = 0; $child = $memberEl->child_nodes[$k]; $k++){
									if($child->node_type == 1){
										if($child->node_name == 'name')
											$name = /*utf8_decode*/($child->first_child->node_value);
										else if($child->node_name == 'value')
											$valueEl = $child;
									}
								}
					...
					case 'array':
						...
						$arr = array();
						$dataEl = $typeEl->first_child;
						while($dataEl && ($dataEl->node_type != 1 || $dataEl->node_name != 'data'))
							$dataEl = $dataEl->next_sibling;
						
						if(!$dataEl)
							trigger_error("XML-RPC Parse Error: Expected 'data' element as sole child element of 'array'.");
						
						$valueEl = $dataEl->first_child;
						while($valueEl){
							if($valueEl->node_type == 1){
								#<arrays>s can be recursive, any value may contain an <array> or
								#   any other type, including a <struct>, described above.
								if($valueEl->node_name == 'value')
									array_push($arr, $this->decodeXmlRpc($valueEl));
								else
									trigger_error("XML-RPC Parse Error: Illegal element child '" . $valueEl->node_name . "' of an array's 'data' element.");
							}
							$valueEl = $valueEl->next_sibling;
						}
						return $arr;
					default:
						trigger_error("XML-RPC Parse Error: Illegal element '" . $typeEl->node_name . "' child of the 'value' element.");
				}
			}
		}
		return '';
	...
	function getEscapeSequence_callback($regExMatches){
		if(isset($this->charToJSON[$regExMatches[0]]))
			return $this->charToJSON[$regExMatches[0]];
		return sprintf('\\u00%02x', ord($regExMatches[0]));
	}
	var $charToJSON = array(
		'\\'   => '\\\\',
		'"'    => '\\"',
		'/'    => '\\/',
		"\x08" => '\\b',
	...
	function getEscapedChar_callback($regExMatches){
		...
		switch($c){
			...
			#case '\\': return "\x5C";
			case 'u':
				return RPCServer::unicodeToUtf8(hexdec(substr($regExMatches[0], 2)));
		}
		return $c;
	}
	...
	function stringToType($str){
		...
		if(is_numeric($str))
			return RPCServer::stringToNumber($str);
		return $str;
	}
