 ...
 * function which will be generated in the JavaScript response. The HTTP-Get
 ...
 *
 * var service = new rpc.ServiceProxy("http://www.google.com/calendar/feeds/myemail%40gmail.com/public", {
...
rpc.ServiceProxy = function(serviceURL, options){
	...
	var urlParts = this.serviceURL.match(/^(\w+:)\/\/([^\/]+?)(?::(\d+))?(?:$|\/)/);
	if(urlParts){
		this.isCrossSite = (
	...
	this.isResponseSanitized = true;
	this.authUsername = null;
	this.authPassword = null;
	this.callbackParamName = 'JSON-response-callback';
	this.protocol = 'JSON-RPC';
	this.dateEncoding = 'ISO8601'; //"@ticks@" || "classHinting" || "ASP.NET"
	this.decodeISO8601 = true; //JSON only
	
	//Get the provided options
	if(options instanceof Object){
		if(options.asynchronous !== undefined){
			this.isAsynchronous = !!options.asynchronous;
			if(!this.isAsynchronous && this.isCrossSite)
	...
	if(this.isCrossSite && this.protocol == 'XML-RPC')
	...
	if(this.isCrossSite && !providedMethodList)
	...
	if(providedMethodList)
		this.methodList = providedMethodList;
	else {
		//Introspection must be performed synchronously
		var async = this.isAsynchronous;
		this.isAsynchronous = false;
		this.methodList = this.callMethod("system.listMethods", []);
		this.isAsynchronous = async;
	}
	this.methodList.push('system.listMethods');
	this.methodList.push('system.describe');
	
	//Create local "wrapper" functions which reference the methods obtained above
	for(var methodName, i = 0; methodName = this.methodList[i]; i++){
		...
		//Create a wrapper to this.callMethod with this instance and this methodName bound
			...
			return function(){
				if(this.isAsynchronous){
					if(arguments.length == 1 && arguments[0] instanceof Object){
						call.instance.callMethod(call.methodName,
												 arguments[0].params,
				...
				else return call.instance.callMethod(call.methodName, rpc.toArray(arguments));
			};
		})(this, methodName);
...
rpc.ServiceProxy.setAsynchronous = function(isAsynchronous){
	if(!isAsynchronous && this.isCrossSite)
		throw Error("It is not possible to establish a synchronous connection to a cross-site RPC service.");
	this.isAsynchronous = !!isAsynchronous;
};

rpc.ServiceProxy.prototype.callMethod = function(methodName, params, successHandler, exceptionHandler, completeHandler){
	rpc.requestCount++;
	...
	try {
		...
		if(this.isCrossSite){ //then this.isAsynchronous is implied
			...
			rpc.callbacks['r' + String(rpc.requestCount)] = (function(instance, id){
				...
				return function(response){
					if(response instanceof Object && (response.result || response.error)){	
						response.id = call.id;
						instance.doCallback(response);
					}
					else {//Allow data without response wrapper (i.e. GData)
						instance.doCallback({id: call.id, result: response});
					}
				}
			})(this, rpc.requestCount);
			//rpc.callbacks['r' + String(rpc.requestCount)] = new Function("response", 'response.id = ' + rpc.requestCount + '; this.doCallback(response);');
			
			//Make the request by adding a SCRIPT element to the page
			var script = document.createElement('script');
			script.setAttribute('type', 'text/javascript');
			var src = this.serviceURL +
						'/' + methodName +
						'?' + this.callbackParamName + '=rpc.callbacks.r' + (rpc.requestCount);
			if(params)
		...
		//Calls made with XMLHttpRequest ------------------------------------------------------------
			...
			if(this.protocol == 'XML-RPC'){
				...
				if(params){
					...
					for(var i = 0; i < params.length; i++){
						...
						paramEl.appendChild(this.toXMLRPC(params[i]));
			...
			xhr.open('POST', this.serviceURL, this.isAsynchronous, this.authUsername, this.authPassword);
			if(this.protocol == 'XML-RPC'){
				...
				xhr.onreadystatechange = function(){
					...
					if(xhr.readyState == 4){
						...
						if(xhr.responseXML){
							...
							{
								response.result = this.parseXMLRPC(valueEl);
							}
							else if(valueEl.parentNode.nodeName == 'fault'){
								var fault = this.parseXMLRPC(valueEl);
								response.error = {
							...
							response.id = requestInfo.id; //XML-RPC cannot pass and return request IDs
							instance.doCallback(response);
						}
						//JSON-RPC
						else
							instance.doCallback(this.evalJSON(xhr.responseText, this.isResponseSanitized));
					}
				};
			...
			//Synchronous same-domain call -----------------------------------------------------
				...
				//Send the request
				xhr.send(postData);
				var response = this.evalJSON(xhr.responseText, this.isResponseSanitized);
				...
				rpc.upgradeValuesFromJSON(response);
...
rpc.ServiceProxy.prototype.doCallback = function(response){
	if(typeof response != 'object')
		throw Error('The server did not respond with a JSON-RPC response object.');
	if(!response.id)
...
rpc.ServiceProxy.prototype.evalJSON = function(json, sanitize){ //from Prototype String.evalJSON()
    ...
    throw new SyntaxError('Badly formed JSON string: ' + json + " ... " + err.message);
};
...
rpc.ServiceProxy.prototype.upgradeValuesFromJSON = function(obj){
	...
	for(var key in obj){
		if(!useHasOwn || obj.hasOwnProperty(key)){
			...
			if(typeof obj[key] == 'string'){
				//ISO8601
				if(this.decodeISO8601 && (matches = obj[key].match(/^(?:(\d\d\d\d)-(\d\d)(?:-(\d\d)(?:T(\d\d)(?::(\d\d)(?::(\d\d)(?:\.(\d+))?)?)?)?)?)$/))){
					obj[key] = new Date(0);
				...
				else this.upgradeValuesFromJSON(obj[key]);
			}
		}
	}
};
	...
	switch(typeof value){
		...
		case 'object':
			if(value === null)
				...
				for(var i = 0; i < value.length; i++)
					dataEl.appendChild(this.toXMLRPC(value[i]));
				...
				for(var key in value){
					if(!useHasOwn || value.hasOwnProperty(key)){
						...
						memberEl.appendChild(this.toXMLRPC(value[key]));
...
rpc.ServiceProxy.prototype.parseXMLRPC = function(valueEl){
		...
		if(valueEl.childNodes.item(i).nodeType == 1){
			...
			switch(typeEL.nodeName.toLowerCase()){
				...
				case 'dateTime.iso8601':
					...
					throw Error("XML-RPC Parse Error: The provided value does not matche ISO8601.");
				case 'base64':
				...
				case 'struct':
					...
					for(var memberEl, j = 0; memberEl = typeEL.childNodes.item(j); j++){
						if(memberEl.nodeType == 1 && memberEl.nodeName == 'member'){
							...
							if(name && valueEl)
								obj[name] = this.parseXMLRPC(valueEl);
						}
					}
					return obj;
				case 'array':
					...
					while(valueEl){
						if(valueEl.nodeType == 1){
							...
							if(valueEl.nodeName == 'value')
								arr.push(this.parseXMLRPC(valueEl));
							else
